...
l1.append("Parent")
l1.append("Child")
l1.append("Confidence")
confidence.append(l1)

for comb in itertools.combinations(l,2):
  l1 = []
  if(count[comb[1]] > count[comb[0]]):
    l1.append(tagid[comb[0]])
    l1.append(tagid[comb[1]])
    l1.append(float(comm[comb[0]][comb[1]])/(count[comb[0]]))
  else:
    l1.append(tagid[comb[1]])
    l1.append(tagid[comb[0]])
  # print(comb[0],comb[1])
    l1.append(float(comm[comb[0]][comb[1]])/(count[comb[1]]))
  confidence.append(l1)
     
with open("confidence.csv", 'w') as csvfile:    
  csvwriter = csv.writer(csvfile)    
  csvwriter.writerows(confidence)
